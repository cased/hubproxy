...
// Dialect implements PostgreSQL-specific SQL dialect
type Dialect struct {
	*sql.BaseDialect
}
...
// PlaceholderFormat returns "$" as PostgreSQL uses $n for placeholders
func (d *Dialect) PlaceholderFormat() string {
	return "$"
}

// JSONType returns JSONB as PostgreSQL's JSON type
func (d *Dialect) JSONType() string {
	return "JSONB"
}

// TimeType returns TIMESTAMP as PostgreSQL's timestamp type
func (d *Dialect) TimeType() string {
	return "TIMESTAMP"
}

// CreateTableSQL returns PostgreSQL-specific table creation SQL
